...
public class DateTimeIteratorFactory {
     ...
     * @param tzid the local timezone -- used to interpret start and any dates
     *            in RDATE and EXDATE lines that don't have TZID params.
    ...
    public static DateTimeIterator createDateTimeIterator(String rdata, ReadableDateTime start, DateTimeZone tzid, boolean strict) throws ParseException {
        return new RecurrenceIteratorWrapper(RecurrenceIteratorFactory.createRecurrenceIterator(rdata, dateTimeToDateValue(start.toDateTime().withZone(tzid)),
                                                                                                TimeZoneConverter.toTimeZone(tzid), strict));
    }
    
    public static DateTimeIterator createDateTimeIterator(Recurrence rdata, ReadableDateTime start, DateTimeZone tzid, boolean strict) throws ParseException {
        return new RecurrenceIteratorWrapper(RecurrenceIteratorFactory.createRecurrenceIterator(rdata, dateTimeToDateValue(start.toDateTime().withZone(tzid)),
                                                                                                TimeZoneConverter.toTimeZone(tzid), strict));
    }
    ...
    public static DateTimeIterable createDateTimeIterable(String rdata, ReadableDateTime start, DateTimeZone tzid, boolean strict) throws ParseException {
        return new RecurrenceIterableWrapper(RecurrenceIteratorFactory.createRecurrenceIterable(rdata, dateTimeToDateValue(start.toDateTime().withZone(tzid)),
                                                                                                TimeZoneConverter.toTimeZone(tzid), strict));
    }
    
    public static DateTimeIterable createDateTimeIterable(Recurrence rdata, ReadableDateTime start, DateTimeZone tzid, boolean strict) throws ParseException {
        return new RecurrenceIterableWrapper(RecurrenceIteratorFactory.createRecurrenceIterable(rdata, dateTimeToDateValue(start.toDateTime().withZone(tzid)),
                                                                                                TimeZoneConverter.toTimeZone(tzid), strict));
    }
    /**
    ...
    private static final class RecurrenceIteratorWrapper implements DateTimeIterator {
